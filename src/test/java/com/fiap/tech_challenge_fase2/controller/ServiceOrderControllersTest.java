...
class ServiceOrderControllersTest {
    ...
    @Test
    @DisplayName("POST /service-orders — deve retornar 201 com OS criada")
    ...
    @Test
    @DisplayName("POST /service-orders — deve retornar 400 quando request inválida")
    ...
    @Test
    @DisplayName("GET /service-orders/{id}/status — deve retornar status da OS")
    void shouldReturnServiceOrderStatus() throws Exception {
        ServiceOrder mock = buildMockOrder();
        when(getStatusUseCase.execute(mock.getId())).thenReturn(mock);

        mockMvc.perform(get("/service-orders/{id}/status", mock.getId()))
    ...
    @Test
    @DisplayName("GET /service-orders/{id}/status — deve retornar 404 quando não encontrada")
    void shouldReturn404WhenNotFound() throws Exception {
        when(getStatusUseCase.execute("id-inexistente"))
                .thenThrow(new ServiceOrderNotFoundException("id-inexistente"));

        mockMvc.perform(get("/service-orders/{id}/status", "id-inexistente"))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.title").value("OS não encontrada"));
    }

    @Test
    @DisplayName("GET /service-orders — deve retornar lista de OS")
    ...
    @Test
    @DisplayName("PATCH /service-orders/{id}/status — deve atualizar status")
    void shouldUpdateStatus() throws Exception {
        ...
        mockMvc.perform(patch("/service-orders/{id}/status", mock.getId())
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    @Test
    @DisplayName("GET /quotations/{id}?token=X&approved=true — deve aprovar orçamento")
    void shouldApproveQuotation() throws Exception {
        ...
        mockMvc.perform(get("/quotations/{id}", mock.getId())
    ...
    @Test
    @DisplayName("GET /quotations/{id}?token=X&approved=false — deve recusar orçamento")
